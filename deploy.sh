...
# ── Step 5b: sync Genie space ID from job output ─────────────────────────
# The notebook sets genie_space_id as a task value. If it differs from
# what was in the config (e.g. first deploy, or space was recreated),
# write the new ID back to databricks.local.yml and _bundle_config.py.
echo "==> Syncing Genie space ID from job output..."
_NEW_GENIE_ID=$(databricks api get "/api/2.1/jobs/runs/get?run_id=${SETUP_RUN_ID}" \
    "${PROFILE_ARGS[@]}" --output json 2>/dev/null \
    | python3 -c "
import sys, json
data = json.load(sys.stdin)
for t in data.get('tasks', []):
    if t['task_key'] == 'prepare_app_infrastructure':
        for tv in t.get('run_output', {}).get('task_values', []):
            if tv.get('key') == 'genie_space_id':
                print(tv['value']); break
        break
" 2>/dev/null || echo "")

if [ -n "$_NEW_GENIE_ID" ] && [ "$_NEW_GENIE_ID" != "$GENIE_SPACE_ID" ]; then
    echo "    Genie space ID changed: ${GENIE_SPACE_ID:-<empty>} → ${_NEW_GENIE_ID}"
    GENIE_SPACE_ID="$_NEW_GENIE_ID"

    # Update databricks.local.yml
    _LOCAL_YML="${SCRIPT_DIR}/databricks.local.yml"
    if [ -f "$_LOCAL_YML" ]; then
        python3 - "$_LOCAL_YML" "$_NEW_GENIE_ID" <<'PYEOF'
import re, sys
path, new_id = sys.argv[1], sys.argv[2]
with open(path) as f:
    content = f.read()
# Replace existing genie_space_id value (quoted or unquoted)
updated = re.sub(
    r'(genie_space_id:\s*)"[^"]*"',
    f'\\1"{new_id}"',
    content,
...
if updated != content:
    with open(path, "w") as f:
        f.write(updated)
    print(f"    Updated genie_space_id in databricks.local.yml")
else:
    print("    WARNING: Could not find genie_space_id in databricks.local.yml")
PYEOF
    fi

    # Regenerate _bundle_config.py with the new ID
    cat > "${SCRIPT_DIR}/app/_bundle_config.py" << CFGEOF
# Auto-generated by deploy.sh — do not edit or commit
CATALOG = '${CATALOG}'
DATA_SCHEMA = '${DATA_SCHEMA}'
MODELS_SCHEMA = '${MODELS_SCHEMA}'
...
CFGEOF
    ...
    # Re-sync _bundle_config.py to workspace
    if [ -n "$APP_SOURCE_PATH" ]; then
        databricks workspace import "${APP_SOURCE_PATH}/_bundle_config.py" \
            --file "${SCRIPT_DIR}/app/_bundle_config.py" \
            --format AUTO --overwrite \
            "${PROFILE_ARGS[@]}" 2>/dev/null \
            && echo "    _bundle_config.py re-synced with Genie space ID" \
            || echo "    WARNING: Could not re-sync _bundle_config.py"
    fi
elif [ -n "$_NEW_GENIE_ID" ]; then
    echo "    Genie space ID unchanged: ${GENIE_SPACE_ID}"
else
    echo "    No Genie space ID found in task output (warehouse_id may be empty)."
fi
