...
# MAGIC %md
# MAGIC # Module 3: Classical Statistical Models at Scale
# MAGIC ## SARIMAX + GARCH(1,1) on Residuals + Monte Carlo with Ray
# MAGIC
# MAGIC Per-segment SARIMAX + GARCH via `applyInPandas`, then Ray-distributed Monte Carlo
# MAGIC (t-Copula + lognormal marginals). Outputs: `predictions_sarima`, `predictions_monte_carlo`,
# MAGIC `predictions_risk_timeline`, `predictions_stress_scenarios`, and registered UC models.
...
# MAGIC
# MAGIC Reads `gold_claims_monthly` (40 segments x 84 months) and `gold_macro_features`
# MAGIC from the Module 1 declarative pipeline.
...
# gold_claims_monthly is produced by the declarative pipeline (task: run_dlt_pipeline in jobs.yml).
...
# MAGIC
# MAGIC Join StatCan indicators (unemployment, HPI growth, housing starts) to the claims
# MAGIC series as SARIMAX exogenous variables.
...
# MAGIC %md
# MAGIC ## 2b. Feature Store Integration
# MAGIC
# MAGIC Join rolling features from `features_segment_monthly` (Module 2) as additional
# MAGIC SARIMAX exogenous variables.
...
# MAGIC %md
# MAGIC ## 3. SARIMAX + GARCH at Scale
# MAGIC
# MAGIC `applyInPandas` distributes per-segment SARIMAX(1,0,1)(1,1,0,12) + GARCH(1,1)
# MAGIC on residuals. Outputs MAPE, conditional volatility, and 12-month forecasts.
...
# MAGIC
# MAGIC Ray-distributed t-Copula + lognormal marginals simulation for a 3-segment portfolio
# MAGIC (Property, Auto, Liability). Produces VaR(99.5%), CVaR, and Expected Loss metrics.
# MAGIC Means and correlations calibrated from `gold_claims_monthly`; CVs from GARCH.
...
# MAGIC
# MAGIC `@ray.remote(num_cpus=1)` tasks: Cholesky + chi2 mixing → t-CDF → lognormal
# MAGIC inverse CDF (Sklar's theorem). 24 concurrent tasks across 4 workers.
...
# MAGIC
# MAGIC 64 tasks dispatched: baseline (4) + 12-month SARIMA-driven VaR (48) + 3 stress
# MAGIC scenarios (12). All run concurrently across 24 Ray CPU slots.
...
with mlflow.start_run(run_name='monte_carlo_portfolio_ray') as run:
    ...
    # Baseline rows → predictions_monte_carlo (backward-compatible with Module 4/app)
    ...
    for _seg_idx, _seg_name in enumerate(["Property", "Auto", "Liability"]):
        ...
        # CLT compound distribution: S = Σ_{i=1}^{N} X_i
        # For X ~ Lognormal(μ, σ): E[X] = exp(μ + σ²/2), Var[X] = (exp(σ²)-1)·exp(2μ+σ²)
        _ex = np.exp(_mu_s + _sig_s**2 / 2)
        _varx = (np.exp(_sig_s**2) - 1) * np.exp(2*_mu_s + _sig_s**2)

        # Conditional on N=n: E[S|N] = n·E[X], Var[S|N] = n·Var[X]
        # Sample S|N ~ Normal(n·E[X], sqrt(n·Var[X])) for each scenario
        ...
        _seg_losses = np.maximum(_seg_losses, 0.0)  # floor at zero
...
# MAGIC
# MAGIC Compare SARIMA forecasts against `gold_reserve_triangle` for reserve adequacy assessment.
...
# MAGIC
# MAGIC Register SARIMA Champion and Monte Carlo Portfolio to UC with `@Champion` alias
# MAGIC for serving endpoint deployment in Module 4.
...
# ── PyFunc wrapper (same interface as Module 4's serving contract) ────────────
...
# MAGIC
# MAGIC **Data lineage:** `gold_claims_monthly` (declarative pipeline) → `silver_rolling_features` → Module 2 → `features_segment_monthly` → SARIMAX exog vars
# MAGIC
