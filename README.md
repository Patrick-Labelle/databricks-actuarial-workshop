...
|----------|-----------|-----------|
| `CATALOG`, `SCHEMA`, `PGDATABASE` | `app/_bundle_config.py` (generated by `deploy.sh`) | `scripts/gen_bundle_config.py` |
| `PGHOST` | `valueFrom: database` in `app/app.yaml` | Injected at runtime from Lakebase resource |
| `ENDPOINT_NAME` | `valueFrom: serving-endpoint` in `app/app.yaml` | Injected at runtime from serving endpoint resource |
