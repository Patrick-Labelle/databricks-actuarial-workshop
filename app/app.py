...
# CATALOG / SCHEMA / PG_DATABASE / ENDPOINT_NAME come from app/_bundle_config.py,
# generated by deploy.sh before each deploy with the resolved bundle variable values.
#
# NOTE: PG_DATABASE is intentionally NOT overridable via the PGDATABASE env var.
# The Databricks Apps runtime injects PGDATABASE=postgres (from the 'database' resource
# which uses database_name: "postgres" to pass deploy-time validation). Letting that
# env var take precedence would cause the app to connect to the wrong database.
try:
    from _bundle_config import CATALOG as _BC_CATALOG, SCHEMA as _BC_SCHEMA, PG_DATABASE as _BC_PG, ENDPOINT_NAME as _BC_ENDPOINT
except ImportError:
    _BC_CATALOG, _BC_SCHEMA, _BC_PG, _BC_ENDPOINT = "my_catalog", "actuarial_workshop", "actuarial_workshop_db", "actuarial-workshop-sarima-forecaster"

CATALOG             = os.environ.get("CATALOG") or _BC_CATALOG
SCHEMA              = os.environ.get("SCHEMA") or _BC_SCHEMA
PG_DATABASE_DEFAULT = _BC_PG   # always from _bundle_config.py — see note above
...
def get_lakebase_conn():
    ...
    database = PG_DATABASE_DEFAULT   # always _bundle_config.py; see constant definition above
